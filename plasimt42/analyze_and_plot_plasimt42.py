...
for n_ens in [2,4,10,20, 100]:
    for pert_scale in [0.001,0.003,0.01, 0.03,0.1,0.3,1, 3, 6, 10, 20]:
        ifile = f'output/plasimt42_leadtime_vs_skill_and_spread_plasimt42_100_0_100_1_1_1_n_svs10_n_ens{n_ens}_pertscale{pert_scale}_nresample1_svdleadtime2.pkl'
...
for i,row in df_best.iterrows():
    ...
    # this only makes sense for _n_ens < =20, since only for these cases do we have
    # meaningful ensmean data, the rest is filled up with the values from n_ens=20.
    # howver, for n_ens=100, I had to rerun the script, and then it was filled up with the data from n_ens=2,
    # therefore, here we do the check only for _n_ens<=20 (the rest is discarded anyway).
    if _n_ens <=20:
...
plt.ylim(ymax=120)
plt.savefig(f'{plotdir}/plasimt42_leadtime_vs_skill_and_spread_best_n_ens{n_ens}.svg')
...
sns.despine()
plt.title(f'n_ens:{n_ens}')
plt.savefig(f'{plotdir}/plasimt42_skill_spread_corr_best_n_ens{n_ens}.svg')
...
sns.despine()
plt.title(f'n_ens:{n_ens}')
plt.savefig(f'{plotdir}/plasimt42_skill_spread_diffcorr_best_n_ens{n_ens}.svg')
...
sub_netens = sub_df.query('n_ens<=10') # netens we only have up to 20 members (the rest are dubplicates from 10 members)
...
plt.title(f'leadtime={leadtime} h')
sns.despine()
plt.savefig(f'{plotdir}/plasimt42_n_ens_vs_corr_leadtime{leadtime}.svg')
...
plt.ylabel('spread error correlation')
sns.despine()
plt.savefig(f'{plotdir}/plasimt42_pert_leadtime_vs_corr_spaghetti.svg')
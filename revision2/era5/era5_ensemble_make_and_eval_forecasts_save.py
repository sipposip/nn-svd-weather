...
def compute_mse(x,y):
    '''mse per sample/forecast, with area weighting'''
    assert(x.shape == y.shape)
    assert(x.shape[1:] == (Nlat,Nlon,n_channels_in))
    error = x - y
    weights_lat = np.cos(np.deg2rad(lat))
    weights_lat /= weights_lat.mean()
    # lat is 2nd dimension , we have [time,lat,lon,channel].
    # therefore we have to add 2 empty dimensions for the weights in order
    # to use broadcasting correctly
    mse = ((error) ** 2 * weights_lat[:,None,None]).mean(axis=(1,2)) * norm_std**2
    return mse
...
for n_ens in [2,4,10,20,100]:
    ...
    if n_ens <=20:
        ...
        for ilead, fc_step in enumerate(range(0, max_forecast_steps)):
            ...
            # save forecast
            np.savez_compressed(f'{outdir}/predictions_netens_ensmean_{param_string}_{test_startyear}-{test_endyear}_nresample{n_resample}_n_ens{n_ens}_leadtime{leadtime}.npz', y_pred_ensmean_netens)
        ...
        pickle.dump(out, open(
            f'{outdir}/era5_fc_eval_results_netens_{param_string}_{test_startyear}-{test_endyear}_nresample{n_resample}_n_ens{n_ens}_rev2.pkl',
            'wb'))
    ...
    for pert_scale in [0.001,0.003,0.01, 0.03,0.1,0.3,1, 3]:
        ...
        for n_svs_reduced in [10,20,40,60,80,100]:
            ...
            for ilead,fc_step in enumerate(range(0,max_forecast_steps)):
                ...
                # save forecast
                np.savez_compressed(f'{outdir}/predictions_svd_ensmean_{param_string}_{test_startyear}-{test_endyear}_{svd_params}_n_svs_reduced{n_svs_reduced}_leadtime{leadtime}.npz', y_pred_ensmean)
                np.savez_compressed(f'{outdir}/predictions_ctrl_{param_string}_{test_startyear}-{test_endyear}_{svd_params}_n_svs_reduced{n_svs_reduced}_leadtime{leadtime}.npz', y_pred_ctrl)
            ...
            pickle.dump(out, open(
                f'{outdir}/era5_fc_eval_results_svd_{param_string}_{test_startyear}-{test_endyear}_{svd_params}_n_svs_reduced{n_svs_reduced}_rev2.pkl',
                'wb'))
    ...
    for pert_scale in [0.001, 0.003, 0.01, 0.03, 0.1, 0.3, 1, 3]:
        ...
        for ilead, fc_step in enumerate(range(0, max_forecast_steps)):
            ...
            # save forecast
            np.savez_compressed(f'{outdir}/predictions_rand_ensmean_{param_string}_{test_startyear}-{test_endyear}_n_ens{n_ens}_pertscale{pert_scale}_nresample{n_resample}_leadtime{leadtime}.npz', y_pred_ensmean_rand)
        ...
        pickle.dump(out, open(
            f'{outdir}/era5_fc_eval_results_rand_{param_string}_{test_startyear}-{test_endyear}_n_ens{n_ens}_pertscale{pert_scale}_nresample{n_resample}_rev2.pkl',
            'wb'))

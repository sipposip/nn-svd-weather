...
for n_ens in [2,4,10,20,100]:
    for pert_scale in [0.001,0.003,0.01, 0.03,0.1,0.3,1, 3]: #TODO: rerun with pertscale 3
...
res_df = []
for sub in full_res:
    # for each leadtime, compute mean error, spread, and spread error correlation, the latter
    ...
    for ltime in range(len(sub['leadtime'])):
        ...
        if ltime > 0:  # for leadtime 0, correlation does not make sense
            # we want the correlation between rmse and spread, so we have to take the sqrt since we have
            # mse and variance (even though it is called "spread_***", it is in fact the variance
...
for i,row in df_best.iterrows():
    ...
    # now update this row with the new variables, and add it to the list of rows for the new dataframe
...
plt.plot(sub_df['leadtime'], sub_df['rmse_ensmean_rand'], label='rmse ensmean rand', color='#7570b3', zorder=1)
plt.plot(sub_df['leadtime'], sub_df['rmse_ensmean_netens'], label='rmse ensmean netens', color='#d95f02')
plt.plot(sub_df['leadtime'], sub_df['spread_svd'], label='spread svd', color='#1b9e77', linestyle='--', zorder=1)
plt.plot(sub_df['leadtime'], sub_df['spread_rand'], label='spread rand', color='#7570b3', linestyle='--', zorder=0)
...
sns.despine()
plt.title(f'n_ens:{n_ens}')
plt.savefig(f'{plotdir}/era5_leadtime_vs_skill_and_spread_diff_best_n_ens{n_ens}.svg')
...
sns.despine()
plt.title(f'n_ens:{n_ens}')
plt.savefig(f'{plotdir}/era5_skill_spread_diffcorr_best_n_ens{n_ens}.svg')
...
plt.axhline(0, color='black', zorder=-5, linewidth=1)
sns.despine()
plt.savefig(f'{plotdir}/era5_n_ens_vs_skill_diff_leadtime{leadtime}.svg')
